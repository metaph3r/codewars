...
export function brainLuck(code: string, input: string): string {
  ...
  const tape: number[] = new Array(30000);
  ...
  while (instructionPointer < code.length) {
    const instruction = code.charAt(instructionPointer++);
    ...
    switch (instruction) {
      ...
      case ">":
        ...
        break;
      case ",":
        tape[dataPointer] = input.charCodeAt(inputPointer++);
        break;
      case ".":
      ...
      case "+":
        tape[dataPointer]++;
        if (tape[dataPointer] > 255) {
          tape[dataPointer] -= 256;
        }
        break;
      case "-":
        tape[dataPointer]--;
      ...
      case "[":
        if (tape[dataPointer] === 0) {
          ...
          while (code.charAt(instructionPointer) !== "]" || opening !== 0) {
      ...
      case "]":
        if (tape[dataPointer] !== 0) {
          ...
          while (code.charAt(instructionPointer) !== "[" || closing !== 0) {
            if (code.charAt(instructionPointer) === "]") {
              closing++;
            } else if (code.charAt(instructionPointer) === "[") {
              closing--;
